...
#
#   Date of last update: September 03, 2021
#
...
#     print ( "Empty" )
#
#--------------------ADDING camels_category FILES--------------------
#
#clear anything already in the table
cur.execute("""DELETE FROM camels_category;""")
# Insert data from the CSV files into the table
print ( "\nadding data to camels_category" )
years = ["2003","2004","2005","2006","2007","2008","2009","2010","2011","2012","2013","2014","2015","2016","2017","2018","2019","2020"]
for year in years:
    with open(year+"_camels_category.csv", 'r') as row:
        cur.copy_from(row, 'camels_category', sep=',')
        print(year+" files added")
#
# Commit data insertion
    conn.commit()
#
#--------------------ADDING camels_usage FILES--------------------
#
#clear anything already in the tables
cur.execute("""DELETE FROM camels_usage;""")
# Insert data from the CSV files into the table
print ( "\nadding data to camels_usage" )
for year in years:
    with open(year+"_camels_usage.csv", 'r') as row:
        cur.copy_from(row, 'camels_usage', sep=',')
        print(year+" files added")
#
# Commit data insertion
    conn.commit()
#
#--------------------ADDING camels_health FILES--------------------
#
#clear anything already in the table
cur.execute("""DELETE FROM camels_health;""")
# Insert data from the CSV files into the table
print ( "\nadding data to camels_health" )
for year in years:
    with open(year+"_camels_health.csv", 'r') as row:
        cur.copy_from(row, 'camels_health', sep=',')
        print(year+" files added")
#
# Commit data insertion
    conn.commit()
#
#--------------------ADDING camels_dairy FILES--------------------
#
#clear anything already in the table
cur.execute("""DELETE FROM camels_dairy;""")
# Insert data from the CSV files into the table
print ( "\nadding data to camels_dairy" )
for year in years:
    with open(year+"_camels_dairy.csv", 'r') as row:
        cur.copy_from(row, 'camels_dairy', sep=',')
        print(year+" files added")
#
# Commit data insertion
    conn.commit()
#
#--------------------ADDING camels_holdings FILES--------------------
#
#clear anything already in the table
cur.execute("""DELETE FROM camels_holdings;""")
# Insert data from the CSV files into the table
print ( "\nadding data to camels_holdings" )
years.remove("2003")
for year in years:
    with open(year+"_camels_holdings.csv", 'r') as row:
        cur.copy_from(row, 'camels_holdings', sep=',')
        print(year+" files added")
#
# Commit data insertion
    conn.commit()
#
#--------------------ADDING camels_estimation FILES--------------------
#
#clear anything already in the table
cur.execute("""DELETE FROM camels_estimation;""")
# Insert data from the CSV files into the table
print ( "\nadding data to camels_estimation" )
years.remove("2004")
for year in years:
    with open(year+"_camels_estimation.csv", 'r') as row:
        cur.copy_from(row, 'camels_estimation', sep=',')
        print(year+" files added")
#
# Commit data insertion
    conn.commit()
