...
# 
# Actions:
# - creates the appropriate Sheep files to upload into the database 
#
# Libraries
import sys
import getopt
...
def main ( argv ):
    try:
        ...
        print ( "createSheepDatabaseFiles.py" )
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-h':
            print ( "createSheepDatabaseFiles.py" )
...
#--------------CREATE year_sheep_category FROM year_Table_3-3 and year_Table_3-15--------------
        rfile1 = open(year+"_Table_3-3.csv", "r")
        rlist1 = list(csv.reader(rfile1, delimiter=','))
        rfile2 = None
        if year == "2003":
            rfile2 = open(year+"_Table_3-19.csv", "r")
        else:
            rfile2 = open(year + "_Table_3-15.csv", "r")
        rlist2 = list(csv.reader(rfile2, delimiter=','))
        #open file for writing
        wfile = open(year + "_sheep_category.csv", "w")
...
#--------------CREATE year_sheep_usage FROM year_Table_3-8--------------
        rfile1 = open(year+"_Table_3-8.csv", "r")
        rlist1 = list(csv.reader(rfile1, delimiter=','))
        #open file for writing
        wfile = open(year + "_sheep_usage.csv", "w")

        for i in range(1, len(rlist1)):
            id = str(getId(idDict, rlist1[i][:2]))
            print(id+","+year+","+rlist1[i][2]+","+rlist1[i][3]+","+rlist1[i][4]+","+rlist1[i][5]+","+rlist1[i][6]+","+rlist1[i][7]+","+rlist1[i][8]+","+rlist1[i][9]+","+rlist1[i][10] , file=wfile)

        rfile1.close()
        wfile.close()

#--------------CREATE year_sheep_estimation FROM year_Table_3-19b and year_Table_3-20b--------------
        if int(year) >= 2005:
            rfile1 = open(year+"_Table_3-19b.csv", "r")
            rlist1 = list(csv.reader(rfile1, delimiter=','))
            rfile2 = open(year + "_Table_3-20b.csv", "r")
            rlist2 = list(csv.reader(rfile2, delimiter=','))
            #open file for writing
            wfile = open(year + "_sheep_estimation.csv", "w")
...
#--------------CREATE year_sheep_health--------------
        ...
        if year == "2003":
            ...
            rfile1 = open(year+"_Table_3-23b.csv", "r")
            rfile2 = open(year + "_Table_3-20b.csv", "r")
            rfile3 = open(year + "_Table_3-24.csv", "r")
        ...
        #open file for writing
        wfile = open(year + "_sheep_health.csv", "w")
        #print differently based on year
        for i in range(1, len(rlist1)):
            id = str(getId(idDict, rlist1[i][:2]))
            if year == "2003":
                print(id+","+year+","+rlist2[i][3]+","+rlist3[i][3]+",0,0,"+rlist1[i][3]+",0,0,0,0,0,0,"+rlist4[i][3]+",0,0" , file=wfile)
            elif year == "2004":
                print(id+","+year+","+rlist2[i][3]+","+rlist3[i][3]+","+rlist4[i][3]+","+rlist5[i][3]+","+rlist1[i][3]+",0,0,0,0,0,0,0,0,0" , file=wfile)
            elif int(year) >= 2006 and int(year) <= 2009:
                print(id+","+year+","+rlist3[i][3]+","+rlist4[i][3]+","+rlist5[i][3]+","+rlist6[i][3]+","+rlist1[i][2]+","+rlist1[i][3]+","+rlist1[i][4]+","+rlist1[i][5]+","+rlist1[i][6]+","+rlist1[i][7]+","+rlist1[i][8]+","+rlist2[i+1][8]+","+rlist2[i+1][9]+","+rlist2[i+1][10] , file=wfile)
            elif year == "2020":
                print(id+","+year+","+rlist3[i][3]+","+rlist4[i][3]+","+rlist5[i][3]+","+rlist6[i][3]+","+rlist1[i][2]+","+rlist1[i][3]+","+rlist1[i][4]+","+rlist1[i][5]+","+rlist1[i][6]+",0,0,"+rlist2[i+1][8]+","+rlist2[i+1][9]+","+rlist2[i+1][10] , file=wfile)
            else:
                print(id+","+year+","+rlist3[i][3]+","+rlist4[i][3]+","+rlist5[i][3]+","+rlist6[i][3]+","+rlist1[i][2]+","+rlist1[i][3]+","+rlist1[i][4]+","+rlist1[i][5]+","+rlist1[i][6]+",0,"+rlist1[i][7]+","+rlist2[i+1][8]+","+rlist2[i+1][9]+","+rlist2[i+1][10] , file=wfile)
...
#--------------CREATE year_sheep_holdings FROM year_Table_3-27b, 2004_Table_3-26b--------------
        if int(year) >= 2004:
            if year == "2004":
                rfile1 = open(year+"_Table_3-26b.csv", "r")
            else:
                rfile1 = open(year+"_Table_3-27b.csv", "r")
            rlist1 = list(csv.reader(rfile1, delimiter=','))
            #open file for writing
            wfile = open(year + "_sheep_holdings.csv", "w")
            #print differently based on year
            for i in range(1, len(rlist1)):
                id = str(getId(idDict, rlist1[i][:2]))
                if int(year) >= 2019:
...
#
# End of createSheepDatabaseFiles.py
#