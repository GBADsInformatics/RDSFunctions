...
#--------------CREATE idtable along with id dictionary--------------
    ...
    for y in range(startYear, endYear):
        year = str(y)
#--------------CREATE year_camels_category FROM year_Table_3-6--------------
        rfile1 = open(year+"_Table_3-6.csv", "r")
        rlist1 = list(csv.reader(rfile1, delimiter=','))
        #open file for writing
        wfile = open(year + "_camels_category.csv", "w")

        for i in range(2, len(rlist1)):
            id = str(getId(idDict, rlist1[i][:2]))
            print(id+","+year+","+rlist1[i][7]+","+rlist1[i][8]+","+rlist1[i][9]+","+rlist1[i][10]+","+rlist1[i][11] , file=wfile)

        rfile1.close()
        wfile.close()

#--------------CREATE year_camels_usage FROM year_Table_3-13--------------
        rfile1 = open(year+"_Table_3-13.csv", "r")
        rlist1 = list(csv.reader(rfile1, delimiter=','))
        #open file for writing
        wfile = open(year + "_camels_usage.csv", "w")
...
#--------------CREATE year_camels_estimation FROM year_Table_3-19g and year_Table_3-20g--------------
        if int(year) >= 2005:
            rfile1 = open(year+"_Table_3-19g.csv", "r")
            rlist1 = list(csv.reader(rfile1, delimiter=','))
            rfile2 = open(year + "_Table_3-20g.csv", "r")
            rlist2 = list(csv.reader(rfile2, delimiter=','))
            #open file for writing
            wfile = open(year + "_camels_estimation.csv", "w")
...
#--------------CREATE year_camels_health--------------
        ...
        if year == "2003":
            ...
            if year == "2005" or year == "2006":
                rfile1 = open(year+"_Table_3-23e.csv", "r")
            else: 
                rfile1 = open(year+"_Table_3-23d.csv", "r")
            rfile2 = open(year + "_Table_3-20g.csv", "r")
            rfile3 = open(year + "_Table_3-24.csv", "r")
        ...
        #open file for writing
        wfile = open(year + "_camels_health.csv", "w")
        #print differently based on year
        for i in range(1, len(rlist1)):
            id = str(getId(idDict, rlist1[i][:2]))
            if year == "2003":
                print(id+","+year+","+rlist2[i][6]+","+rlist3[i][6]+",0,0,"+rlist1[i][6]+",0,0,0,0,0,0,"+rlist4[i][6]+",0,0" , file=wfile)
            elif year == "2004":
                print(id+","+year+","+rlist2[i][6]+","+rlist3[i][6]+","+rlist4[i][6]+","+rlist5[i][6]+","+rlist1[i][6]+",0,0,0,0,0,0,0,0,0" , file=wfile)
            elif int(year) >= 2006 and int(year) <= 2009:
                print(id+","+year+","+rlist3[i][6]+","+rlist4[i][6]+","+rlist5[i][8]+","+rlist6[i][8]+","+rlist1[i][2]+","+rlist1[i][3]+","+rlist1[i][4]+","+rlist1[i][5]+","+rlist1[i][6]+","+rlist1[i][7]+","+rlist1[i][8]+","+rlist2[i+1][8]+","+rlist2[i+1][9]+","+rlist2[i+1][10] , file=wfile)
            elif year == "2020":
                print(id+","+year+","+rlist3[i][6]+","+rlist4[i][6]+","+rlist5[i][8]+","+rlist6[i][8]+","+rlist1[i][2]+","+rlist1[i][3]+","+rlist1[i][4]+","+rlist1[i][5]+","+rlist1[i][6]+",0,0,"+rlist2[i+1][8]+","+rlist2[i+1][9]+","+rlist2[i+1][10] , file=wfile)
            else:
                print(id+","+year+","+rlist3[i][6]+","+rlist4[i][6]+","+rlist5[i][8]+","+rlist6[i][8]+","+rlist1[i][2]+","+rlist1[i][3]+","+rlist1[i][4]+","+rlist1[i][5]+","+rlist1[i][6]+",0,"+rlist1[i][7]+","+rlist2[i+1][8]+","+rlist2[i+1][9]+","+rlist2[i+1][10] , file=wfile)
...
#--------------CREATE year_camels_dairy FROM year_Table_3-21, 2003_Table_3-16, 2004_Table_3-20--------------
        if year == "2003":
        ...
        rlist1 = list(csv.reader(rfile1, delimiter=','))
        #open file for writing
        wfile = open(year + "_camels_dairy.csv", "w")
...
#--------------CREATE year_camels_holdings FROM year_Table_3-27e, 2004_Table_3-26e--------------
        if int(year) >= 2004:
            if year == "2004":
                rfile1 = open(year+"_Table_3-26e.csv", "r")
            else:
                rfile1 = open(year+"_Table_3-27e.csv", "r")
            rlist1 = list(csv.reader(rfile1, delimiter=','))
            #open file for writing
            wfile = open(year + "_camels_holdings.csv", "w")
            #print differently based on year
            for i in range(2, len(rlist1)):
                id = str(getId(idDict, rlist1[i][:2]))
                # For years without total holding, calculate it and add it to the table
                holdingList = [int(val) for val in rlist1[i][7:]]
                totalHolding = str(sum(holdingList))
                print(id+","+year+","+totalHolding+","+rlist1[i][7]+","+rlist1[i][8]+","+rlist1[i][9]+","+rlist1[i][10]+","+rlist1[i][11] , file=wfile)
...
#
# End of createCamelsCategory.py
#